...
4. [PostgreSQL Guide](docs/rhdh-local-guide/postgresql-guide.md) - using PostgreSQL instead of an in-memory database, including **schema mode** (`pluginDivisionMode: schema`) when the DB user cannot create multiple databases (`CREATEDB`)
