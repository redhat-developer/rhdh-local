...
7. (Optional) A [Red Hat account](https://access.redhat.com/RegistryAuthentication#getting-a-red-hat-login-2), if you pull images from `registry.redhat.io` (commercially supported RHDH, or a RHEL Postgres image via `POSTGRES_IMAGE`). The default Postgres image is public (`quay.io/fedora/postgresql-18`).
