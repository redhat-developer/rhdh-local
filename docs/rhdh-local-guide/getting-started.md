...
7. (Optional) A [Red Hat account](https://access.redhat.com/RegistryAuthentication#getting-a-red-hat-login-2), if you pull images from `registry.redhat.io` (the default Postgres image, or commericially supported RHDH). You can skip this by setting `POSTGRES_IMAGE` in `.env` to a public image.
