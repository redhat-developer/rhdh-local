...
- `LIGHTSPEED_STACK_OKP_CONFIG`: Host path to the OKP-enabled Lightspeed stack YAML used by `compose.okp-enabled.override.example.yaml` (default: `configs/extra-files/lightspeed-stack.yaml`)
- `OKP_IMAGE`: OKP image override (default: the pinned `registry.redhat.io/offline-knowledge-portal/rhokp-rhel9` image in `compose.okp-enabled.override.example.yaml`)
